...
class TB(layers.Layer):
    ...
    def call(self, inputs, training=True, **kwargs):
        return self.ln(self.conv(inputs))
...
class OpticalHierarchicalTransformer(Base, ABC):
    def __init__(
            self,
            patches=8,
            heads=(2, 4, 8, 16),
        ...
        super().__init__(**kwargs)
        self.patches = patches if type(patches) is int else patches[0]
        self.heads = heads
    ...
    def call(self, inputs, training=True, **kwargs):
        ...
        init_channels = self._calc_channels(64, width_scalar=self.width_scalar)
        m = Stem(
        ...
        m = Patchify(patch_size=self.patches)(m)
        m = PatchEncoder(
            num_patches=(inputs.shape[-2]//self.patches)**2,
            embedding_size=init_channels
        )(m)

        for i, (h, r) in enumerate(zip(self.heads, self.repeats)):
            if i > 0:
                m = TB(filters=self._calc_channels(2*m.shape[-1], width_scalar=self.width_scalar))(m)
