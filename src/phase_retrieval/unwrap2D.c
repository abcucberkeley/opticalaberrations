    ...
    int nrhs, const mxArray* prhs[])
{
    ...
    if (nrhs != 8)
        mexErrMsgTxt("Eight input arguments required. In this order: wrapped_image, input_mask, image_width, image_height, wrap_around_x, wrap_around_y, use_seed, seed");

    if (nlhs > 1)
        mexErrMsgTxt("Too many output arguments. Just 1: unwrapped image");
    ...
    unsigned int seed;
        ...
        input_mask = mxGetPr(prhs[1]);
        image_width = mxGetScalar(prhs[2]);
        image_height = mxGetScalar(prhs[3]);
        wrap_around_x = mxGetScalar(prhs[4]);
        wrap_around_y = mxGetScalar(prhs[5]);
        use_seed = mxGetLogicals(prhs[6]);
        seed = mxGetScalar(prhs[7]);

    // create the output matrix
    plhs[0] = mxCreateDoubleMatrix(image_width, image_height, mxREAL);
    UnwrappedImage = mxGetPr(plhs[0]);

    //call the computational routine 
    unwrap2D(wrapped_image, UnwrappedImage, input_mask, image_width, image_height, wrap_around_x, wrap_around_y, use_seed, seed);
}