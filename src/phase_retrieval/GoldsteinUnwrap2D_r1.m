function [im_unwrapped]=GoldsteinUnwrap2D_r1(IM,im_mask)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% mag_max = max(im_mag(:));
% indx1 = find(im_mag < 0.015*mag_max);  %Intensity = mag^2, so this = .04 threshold on the intensity
...
%   colref = colrefn(1);                  %Choose the 1st point for a reference (known good value)
 rowref = floor(r_dim/2);%128;                  %Choose the 1st point for a reference (known good value)
  colref = floor(c_dim/2);%128;                                 %Choose the 1st point for a reference (known good value)
end
