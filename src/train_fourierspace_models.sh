...
EMBEDDING='principle_planes'


MODES=15
BATCH=1024
#python manager.py slurm train.py --partition abc --constraint 'titan' --mem '500GB' --gpus 4 --cpus 16 \
#--task "--network otfnet $PHASE --embedding $EMBEDDING --max_amplitude $MAXAMP --batch_size $BATCH --dataset $DATA/i$SHAPE --input_shape $SHAPE --depth_scalar $DEPTH --modes $MODES --psf_type $PSF_TYPE --wavelength $LAMBDA --x_voxel_size $xVOXEL --y_voxel_size $yVOXEL --z_voxel_size $zVOXEL" \
#--taskname otfnet \
